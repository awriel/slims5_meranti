 ...
 */
 ...
 /* This source modified by Muh Tarom (stuqly@gmail.com) on Tuesday, 25 December 2012*/

/* Book Return Slip print */
...
if (isset($_POST['itemID']) AND !empty($_POST['itemID']) AND isset($_POST['itemAction'])) {
	...
	// loop array
	if (isset($_SESSION['bookslips'])) {
		$print_count = count($_SESSION['bookslips']);
	} else {
		$print_count = 0;
	}
	// barcode size
	...
	foreach ($_POST['itemID'] as $itemID) {
		...
		if (isset($_SESSION['bookslips'][$itemID])) {
			continue;
		}
		if (!empty($itemID)) {
			...
			// add to sessions
			$_SESSION['bookslips'][$itemID] = $itemID;
			$print_count++;
		}
	}
...
if (isset($_GET['action']) AND $_GET['action'] == 'print') {
	...
	$item_ids = '';
	foreach ($_SESSION['bookslips'] as $id) {
		$item_ids .= '\''.$id.'\',';
	}
	...
	$item_q = $dbs->query('SELECT i.item_id, i.biblio_id, i.item_code, i.inventory_code, b.call_number, b.biblio_id, b.title, b.sor FROM item AS i
	...
	foreach ($chunked_book_card_arrays as $bookslip_rows) {
		...
		foreach ($bookslip_rows as $bookslip) {
			$html_str .= '<td valign="top">'."\n";
				...
				if ($bookslip_include_header_text) { $html_str .= '<div class="libname">'.($bookslip_header_text?$bookslip_header_text:$sysconf['library_name']).'<br />Jalan Raya Pucang No. 132, Banjarnegara</div>'."\n";}
	...
	unset($_SESSION['bookslips']);
	// write to file
	$print_file_name = 'book_slip_gen_print_result_'.strtolower(str_replace(' ', '_', $_SESSION['uname'])).'.html';
	$file_write = @file_put_contents(FILES_UPLOAD_DIR.$print_file_name, $html_str);
	if ($file_write) {
		// update print queue count object
		echo '<script type="text/javascript">parent.$(\'#queueCount\').html(\'0\');</script>';
		// open result in window
		echo '<script type="text/javascript">top.openHTMLpop(\''.SENAYAN_WEB_ROOT_DIR.FILES_DIR.'/'.$print_file_name.'\', 800, 500, \''.__('Book Card Printing by Muh Tarom').'\')</script>';
	} else { utility::jsAlert('ERROR! Item book return slip failed to generate, possibly because '.SENAYAN_BASE_DIR.FILES_DIR.' directory is not writable'); }
	exit();
}
...
<div class="menuBoxInner printIcon">
	<div class="per_title">
		<h2><?php echo __('Book Return Slip Printing'); ?></h2>
	</div>
	<div class="sub_section">
		<div class="action_button">
	   <a target="blindSubmit" href="<?php echo MODULES_WEB_ROOT_DIR; ?>bibliography/book_return_slip.php?action=clear" class="notAJAX headerText2" style="color: #f00;"><?php echo __('Clear Print Queue'); ?></a>
	   <a target="blindSubmit" href="<?php echo MODULES_WEB_ROOT_DIR; ?>bibliography/book_return_slip.php?action=print" class="notAJAX headerText2"><?php echo __('Print Book Return Slip for Selected Data');?></a>
		</div>
	  <form name="search" action="<?php echo MODULES_WEB_ROOT_DIR; ?>bibliography/book_return_slip.php" id="search" method="get" style="display: inline;"><?php echo __('Search'); ?> :
	...
	if (isset($_SESSION['bookslips'])) {
		echo '<font id="queueCount" style="color: #f00">'.count($_SESSION['bookslips']).'</font>';
